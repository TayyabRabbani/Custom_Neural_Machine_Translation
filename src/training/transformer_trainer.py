...
class TransformerTrainer:
    def __init__(
        ...
        self.learning_rate = CustomSchedule(d_model=d_model)
        ...
        self.best_weights_path = os.path.join(self.checkpoint_dir, "best.weights.h5")

        self.final_weights_path = os.path.join(self.checkpoint_dir, "final.weights.h5")

        self.history_path = os.path.join(self.checkpoint_dir, "history.json")
    ...
    def validate(self, val_dataset):
        ...
        val_bar = tqdm(val_dataset, desc="Validation", unit="batch")
    ...
    def train(self, train_dataset, val_dataset, epochs=7):

        best_val_loss = float("inf")

        for epoch in range(epochs):

            print(f"\nEpoch " f"{epoch + 1}" f"/{epochs}")
        ...
        self.model.save_weights(self.final_weights_path)
