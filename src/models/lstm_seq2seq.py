...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, hidden, cell, training=False):

        x = self.embedding(x)

        output, h, c = self.lstm(x, initial_state=[hidden, cell], training=training)
...
class Seq2Seq(tf.keras.Model):
    def __init__(self, vocab_size, embedding_dim=256, hidden_units=512):
    ...
    def call(self, inputs, training=False):
        ...
        _, h, c = self.encoder(encoder_inputs, training=training)

        logits, _, _ = self.decoder(decoder_inputs, h, c, training=training)
...
if __name__ == "__main__":
    ...
    dummy_batch = {
        "encoder_inputs": tf.random.uniform(
            (64, 40), maxval=vocab_size, dtype=tf.int32
        ),
        "decoder_inputs": tf.random.uniform(
            (64, 39), maxval=vocab_size, dtype=tf.int32
        ),
    }
